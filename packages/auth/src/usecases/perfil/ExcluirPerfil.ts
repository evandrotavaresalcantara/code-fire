...
export default class ExcluirPerfil implements CasoDeUso<Entrada, void>{
    ...
    async executar(entrada: string): Promise<void> {
        ...
        const existeUsuariosComEssePerfil =  await this.repoUsuario.obterUsuarioPorPerfilId(entrada)
        if(existeUsuariosComEssePerfil) throw new Error('não é possível excluir o perfil. Existe usuário associado a ele.')
