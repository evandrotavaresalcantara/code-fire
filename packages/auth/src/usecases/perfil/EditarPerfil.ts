...
export default class EditarPerfil implements CasoDeUso<Entrada, void> {
    ...
    async executar(entrada: Entrada): Promise<void> {
        ...
        if (entrada.nome) {
            const existePerfilComEsseNome = await this.repoPerfil.obterPerfilPorNome(entrada.nome)
            if (existePerfilComEsseNome) throw new Error('já existe perfil com esse nome.')
        }

        for (const permissao of permissoes) {
            const permissaoBD = await this.repoPermissao.obterPermissaoPorId(permissao)
