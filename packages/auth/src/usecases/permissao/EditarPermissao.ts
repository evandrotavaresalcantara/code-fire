...
export default class EditarPermissao implements CasoDeUso<Entrada, void> {
    ...
    async executar(entrada: Entrada): Promise<void> {
        ...
        const existePermissao = await this.repo.obterPermissaoPorId(idPermissao.uuid)
        if (!existePermissao) throw new Error('permissão não existe.')

        if (entrada.nome) {
            const existePermissaoComEsseNome = await this.repo.obterPermissaoPorNome(entrada.nome)
            if (existePermissaoComEsseNome &&
