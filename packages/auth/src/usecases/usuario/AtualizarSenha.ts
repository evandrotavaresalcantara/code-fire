import { CasoDeUso, CasoDeUsoComUsuario, EntidadeProps, Id } from "common";
...
export default class AtualizarSenha implements CasoDeUso<Entrada, void> {
    ...
    async executar(entrada: Entrada): Promise<void> {
        ...
        const senhaAtualConfirmada = this.provedorCriptografia.comparar(entrada.senhaAntiga, hashSenha)
        
        if(!senhaAtualConfirmada) throw new Error('email ou senha incorreto.')
         
        const hashSenhaNova = this.provedorCriptografia.criptografar(novaSenha.valor)
