...
export class RepositorioPermissaoMock implements RepositorioPermissao {
    ...
    async obterPermissaoPorNome(nome: string): Promise<Permissao | null> {
        return this.permissoes.find((permissao) => permissao.getNomePermissao() === nome
        ) ?? null
    }
    ...
    async obterPermissaoPorId(id: string): Promise<Permissao | null> {
        return this.permissoes.find((p) => p.getUuid() === id) ?? null
    }
    ...
    private _salvar(permissao: Permissao): void {
        const index = this.permissoes.findIndex((p) => p.getUuid() === permissao.getUuid())
        if (index >= 0) {
