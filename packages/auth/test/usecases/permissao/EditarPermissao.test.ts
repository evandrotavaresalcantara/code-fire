...
test('Deve editar uma permissão', async () =>{
    ...
    const novaPermissaoAtualizada = await repo.obterPermissaoPorId(novaPermissao1.getUuid())

    expect(novaPermissaoAtualizada?.getNomePermissao()).toEqual(permissao1Atualizada.nome)
    expect(novaPermissaoAtualizada?.getDescricaoPermissao()).toEqual(permissao1Atualizada.descricao)
})
...
test('Deve editar um permissão com nome já existente se for da própria permissao', async () =>{
    ...
    const permissaoAtualizada = { 
     ...
     expect(novaPermissaoAtualizada?.getNomePermissao()).toEqual(permissaoAtualizada.nome)
     expect(novaPermissaoAtualizada?.getDescricaoPermissao()).toEqual(permissaoAtualizada.descricao)
