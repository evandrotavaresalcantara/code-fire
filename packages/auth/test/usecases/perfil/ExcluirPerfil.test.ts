...
test('gerar um erro ao tentar excluir um perfil com usuário associado', async () => {
    ...
    const novoUsuario = new Usuario({nomeCompleto: 'Usuario', email: 'usuario@io.dev'})
    ...
    }).rejects.toThrow('não é possível excluir o perfil. Existe usuário associado a ele.')
