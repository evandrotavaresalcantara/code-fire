...
test('Deve criar um novo perfil, com uma permissão', async () =>{
    ...
    const perfil1Salvo = await repo.obterPerfilPorNome(perfilComUmaPermissao.nome)
    const permissao = perfil1Salvo?.obterPermissoes.find((p) => p.getUuid() === novaPermissao1.getUuid())
...
test('Deve criar um novo perfil, com duas permissões', async () =>{
    ...
    const repoPermissao = new RepositorioPermissaoMock([novaPermissao1, novaPermissao2])
    const repo = new RepositorioPerfilMock()
    const casoDeUso = new CriarPerfil(repo, repoPermissao)
    ...
    const perfil1Salvo = await repo.obterPerfilPorNome(perfilComUmaPermissao.nome)
    const permissao1Salva = perfil1Salvo?.obterPermissoes.find((p) => p.getUuid() === novaPermissao1.getUuid())
    const permissao2Salva = perfil1Salvo?.obterPermissoes.find((p) => p.getUuid() === novaPermissao2.getUuid())

    expect(permissao1Salva?.getUuid()).toEqual(novaPermissao1.getUuid())
    expect(permissao1Salva?.getNomePermissao()).toBe(novaPermissao1.getNomePermissao())
    expect(permissao2Salva?.getUuid()).toEqual(novaPermissao2.getUuid())
    expect(permissao2Salva?.getNomePermissao()).toBe(novaPermissao2.getNomePermissao())
...
test('Deve criar um novo perfil com duas permissões, mesmo passando uma terceira permissão não cadastrada', async () =>{
    ...
    const perfilComDuasPermissoes = {
        nome: 'Dev', 
        descricao: 'Desenvolvedor',
        permissoes: [novaPermissao1.getUuid(), novaPermissao2.getUuid(), novaPermissao3.getUuid()]
    }
    const repoPermissao = new RepositorioPermissaoMock([novaPermissao1, novaPermissao2])
    const repo = new RepositorioPerfilMock()
    const casoDeUso = new CriarPerfil(repo, repoPermissao)
    ...
    const perfil1Salvo = await repo.obterPerfilPorNome(perfilComDuasPermissoes.nome)
    const permissao1Salva = perfil1Salvo?.obterPermissoes.find((p) => p.getUuid() === novaPermissao1.getUuid())
    const permissao2Salva = perfil1Salvo?.obterPermissoes.find((p) => p.getUuid() === novaPermissao2.getUuid())
